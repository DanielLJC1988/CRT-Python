...
fping = open(r"c:/Python27/MDCsiteCU.txt")
writelog = open("c:/Python27/MDClogCU.txt","a+")
while True:
	...
	if linesite:
		...
		if (outPing.startswith("1 received") or outPing.startswith("2 received") or outPing.startswith("3 received") or outPing.startswith("4 received")):
			...
			if (outPing3.startswith("port 22: Connection refused")):
				...
				crt.Screen.Send("ping 10.125.36.1" + chr(13))
				time.sleep(12)
				...
				if (outPing2.startswith("!!!!!") or outPing2.startswith(".!!!!") or outPing2.startswith("..!!!") or outPing2.startswith(".!!.!") or outPing2.startswith(".!!!.") or outPing2.startswith("!!!!.")):
					...
					time.sleep(0.3)
					crt.Screen.Send("no offset-list OFF_BLINK_OUT out 5000000 Tunnel101 " + chr(13))
					time.sleep(0.3)
					crt.Screen.Send("no offset-list OFF_BLINK_IN in 5000000 Tunnel101 " + chr(13))
					time.sleep(0.3)
					crt.Screen.Send("offset-list OFF_BLINK_OUT out 5000000 Tunnel100 " + chr(13))
					time.sleep(0.3)
					crt.Screen.Send("offset-list OFF_BLINK_IN in 5000000 Tunnel100 " + chr(13))
					time.sleep(0.3)
					...
					writelog.write(pingIP + " tunnel 101 is down!!  so not change tunnel!" + "\n")
					crt.Screen.Send("exit" + chr(13))
					time.sleep(2)
					continue
